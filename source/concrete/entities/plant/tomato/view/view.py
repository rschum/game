    ...
    def on_render(self):
        if self.health_state == HealthState.HEALTHY:
            if self.created_delta() <= 10:
                ...
                if self.growth_state != GrowthState.SAPLING:
                    self.growth_state = GrowthState.SAPLING
                    self.animation = action.Action(healthy_sapling.data)
            elif self.created_delta() <= 40:
                if self.growth_state != GrowthState.MATURE:
                    self.growth_state = GrowthState.MATURE
